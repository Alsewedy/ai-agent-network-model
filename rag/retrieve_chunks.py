"""
retrieve_chunks.py – Baseline connectivity-aware retriever for the new network knowledge architecture.

Design goals:
  1. Retrieve markdown chunks only; structured YAML remains a direct input outside RAG.
  2. Expand focus from scope units to their documented entities using model.yaml.
  3. Score communication-heavy chunks by entity connectivity and scope relevance.
  4. Prioritize exact technical and required-flow chunks for hardening-style questions.
  5. Keep the retriever simpler than v2 while still aligned to the new knowledge structure.
"""
...
INTENT_KEYWORDS = {
    ...
    "uncertainty": [
        ...
        "uncertain", "needs confirmation", "assumption",
    ],
    "dependencies": [
...
# ──────────────────────────────────────────────────────────────
# Model-aware aliases / focus expansion
# ──────────────────────────────────────────────────────────────
...
def score_chunk(question: str, chunk: dict, focus: dict, intents: set[str]) -> dict:
    ...
    if touching_scope_entities and len(touching_entities) >= 2:
...
def compute_dynamic_quotas(intents: set[str], n_relevant_flows: int) -> dict:
    ...
    if "transition_plan" in intents:
        maximums["technical_matrix"] = max(10, n_relevant_flows)
...
def retrieve_top_chunks(question: str, top_k: int = 10, debug: bool = False):
    ...
    if debug:
        selected_ids = {chunk["chunk_id"] for _, chunk in results}
        missed_important = [
            (score, chunk, breakdown)
            for score, chunk, breakdown in scored[:50]
            if chunk["chunk_id"] not in selected_ids
            and chunk["chunk_type"] in ("technical_matrix", "required_flows", "unnecessary_access", "open_questions")
        ]
        if missed_important:
            print("\n--- Scored but not selected (top relevant) ---")
            for score, chunk, _ in missed_important[:10]:
                print(
                    f"  score={score:3d} | {chunk['chunk_type']:18s} | "
                    f"{chunk['chunk_id']} | {chunk['section_title'][:55]}"
                )
...
def main():
    print("RAG Retriever (baseline, new knowledge architecture)")
