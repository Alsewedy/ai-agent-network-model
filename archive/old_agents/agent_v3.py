...
def short_context_snippet(text: str, keyword: str, window: int = 400) -> str:
    if not text:
        return ""
...
def answer_app01_dependencies(model):
    ...
    md_text = load_markdown_text(MARKDOWN_FILES["dependencies"])
    snippet = short_context_snippet(md_text, "## Asset: APP01")

    answer = "APP01 depends on:\n- " + "\n- ".join(deps)
...
def answer_app02_required_flows(model):
    ...
    md_text = load_markdown_text(MARKDOWN_FILES["required_flows"])
    snippet = short_context_snippet(md_text, "## Flow: APP02")

    answer = "Required communication for APP02:\n" + "\n".join(lines)
...
def answer_app01_internal_api_port(model):
    ...
    md_text = load_markdown_text(MARKDOWN_FILES["required_flows"])
    snippet = short_context_snippet(md_text, "## Flow: APP01 -> Internal API on IAM01")
...
def answer_app_zone_comparison(model):
    ...
    md_text = load_markdown_text(MARKDOWN_FILES["target_intent"])
    snippet = short_context_snippet(md_text, "### APP_ZONE")
...
def answer_open_questions(model):
    ...
    md_text = load_markdown_text(MARKDOWN_FILES["open_questions"])
    snippet = short_context_snippet(md_text, "## Open Questions")
